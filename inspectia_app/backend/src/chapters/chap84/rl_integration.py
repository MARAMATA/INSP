# backend/src/chapters/chap84/rl_integration.py
...
_MANAGERS = {
    ...
    "expert":  AdvancedRLManager(chapter="chap84", epsilon=0.02, strategy="hybrid"),
}
...
def rl_add_feedback(
    ...
    *,
    ...
    try:
        ...
        if retraining_system.should_retrain("chap84"):
            retraining_result = retraining_system.retrain_model("chap84")
...
def rl_get_inspector_profiles() -> Dict[str, Any]:
    """Récupère tous les profils d'inspecteurs pour le chapitre 84"""
    try:
...
def rl_retraining_status() -> Dict[str, Any]:
    """Récupère le statut de retraining pour le chapitre 84"""
    try:
        retraining_system = get_retraining_system()
        return {
            "chapter": "chap84",
            "should_retrain": retraining_system.should_retrain("chap84"),
            "feedback_count": retraining_system._get_feedback_count("chap84"),
            "feedback_quality": retraining_system._get_feedback_quality("chap84"),
...
def rl_trigger_retraining() -> Dict[str, Any]:
    """Déclenche manuellement le retraining pour le chapitre 84"""
    try:
        retraining_system = get_retraining_system()
        return retraining_system.retrain_model("chap84")
    except Exception as e:
