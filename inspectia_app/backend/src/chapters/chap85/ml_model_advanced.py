#!/usr/bin/env python3
"""
Modèle ML Supervisé pour la Détection de Fraude - Chapitre 85 AVANCÉ
...
class Chap85MLAdvanced:
    """Modèle ML Supervisé pour la Détection de Fraude - Chapitre 85 AVANCÉ"""
    
    def __init__(self):
        # Chemins des données
        self.backend_root = Path(__file__).resolve().parents[3]
        self.data_path = self.backend_root / "data/processed/CHAP85_PROCESSED_ADVANCED.csv"
        self.splits_path = self.backend_root / "data/ml_splits/chap85"
        self.results_path = self.backend_root / "results/chap85"
        ...
        # Configuration des modèles - ASTUCES POUR GROS DATASETS
        self.models = {
            'RandomForest': RandomForestClassifier(
                ...
                random_state=42,
                n_jobs=-1,
                # ASTUCE: Utiliser bootstrap=False pour économiser la mémoire
                bootstrap=False,
                # ASTUCE: Limiter les features à chaque split
                max_features='sqrt'
            ),
            'LogisticRegression': LogisticRegression(
                ...
                # ASTUCE: Utiliser solver plus rapide
                solver='liblinear'
            ),
            'LightGBM': lgb.LGBMClassifier(
                ...
                n_jobs=-1,
                class_weight='balanced',
                # ASTUCE: Optimisations mémoire LightGBM
                force_col_wise=True,
                force_row_wise=False,
                # ASTUCE: Réduire la taille des bins
                max_bin=255
            ),
            'XGBoost': xgb.XGBClassifier(
                ...
                n_jobs=-1,
                eval_metric='logloss',
                # ASTUCE: Optimisations mémoire XGBoost
                tree_method='hist',
                # ASTUCE: Réduire la taille des bins
                max_bin=256
            ),
            'CatBoost': cb.CatBoostClassifier(
                ...
                random_state=42,
                verbose=False,
                # ASTUCE: Optimisations mémoire CatBoost
                thread_count=-1,
                # ASTUCE: Utiliser moins de RAM
                used_ram_limit='4GB'
            )
        }
        ...
        logger.info("✅ Modèle ML Chapitre 85 AVANCÉ initialisé")
    ...
    def _get_feature_columns(self):
        ...
        # Colonnes business spécifiques au chapitre 85 (Appareils électriques)
        business_features = [
    ...
    def create_preprocessing_pipeline(self, X):
        ...
        numeric_features = [
            ...
            'BUSINESS_POIDS_FAIBLE', 'BUSINESS_DROITS_ELEVES', 'BUSINESS_RATIO_LIQUIDATION_CAF',
            'BUSINESS_RATIO_DOUANE_CAF', 'BUSINESS_IS_ELECTRONIQUE', 'BUSINESS_IS_TELEPHONE',
        ...
        categorical_transformer = Pipeline(steps=[
            ...
            # ASTUCE: Limiter le nombre de catégories pour éviter l'explosion de features
    ...
    def train_models(self, splits, preprocessor):
        """Entraîner tous les modèles en respectant la convention train/val/test avec optimisations mémoire"""
        ...
        for name, model in self.models.items():
            ...
            try:
                # ASTUCE: Nettoyer la mémoire avant chaque entraînement
                import gc
                gc.collect()
                ...
                if len(splits['X_train']) > 100000:
                    logger.info(f"   🔧 Entraînement par chunks pour {name}...")
                ...
                # ASTUCE: Prédictions par chunks pour économiser la mémoire
                chunk_size = 10000
                y_pred_list = []
                y_pred_proba_list = []
                
                for i in range(0, len(splits['X_test']), chunk_size):
                    chunk_X = splits['X_test'].iloc[i:i+chunk_size]
                    chunk_pred = pipeline.predict(chunk_X)
                    chunk_proba = pipeline.predict_proba(chunk_X)[:, 1]
                    y_pred_list.extend(chunk_pred)
                    y_pred_proba_list.extend(chunk_proba)
                
                y_test_pred = np.array(y_pred_list)
                y_test_pred_proba = np.array(y_pred_proba_list)
                ...
                # ASTUCE: Nettoyer la mémoire après chaque modèle
                del y_test_pred, y_test_pred_proba, y_pred_list, y_pred_proba_list
                gc.collect()
    ...
    def _plot_metrics_comparison(self, results, best_model_name):
        ...
        fig.suptitle('Comparaison des Métriques - Chapitre 85 (Appareils Électriques)', 
    ...
    def _plot_confusion_matrices(self, trained_models, splits, best_model_name):
        ...
        fig, axes = plt.subplots(2, 3, figsize=(20, 14))
        fig.suptitle('Matrices de Confusion - Chapitre 85 (Appareils Électriques)', 
                     fontsize=18, fontweight='bold', y=0.98)
        ...
        plt.title(f'Matrice de Confusion - {best_model_name} (Meilleur Modèle)\n'
                 f'Chapitre 85 (Appareils Électriques)\n'
    ...
    def _plot_roc_curves(self, trained_models, splits, best_model_name):
        ...
        plt.title('Courbes ROC - Chapitre 85 (Appareils Électriques)', 
    ...
    def _plot_precision_recall_curves(self, trained_models, splits, best_model_name):
        ...
        plt.title('Courbes Precision-Recall - Chapitre 85 (Appareils Électriques)', 
                 fontsize=16, fontweight='bold', pad=20)
    ...
    def _generate_shap_analysis(self, best_model, splits):
        ...
        try:
            ...
            plt.title('SHAP Feature Importance - Top 20 Features (Chapitre 85)', fontsize=14, fontweight='bold')
            plt.tight_layout()
            ...
            plt.title('SHAP Feature Values - Top 20 Features (Chapitre 85)', fontsize=14, fontweight='bold')
            plt.tight_layout()
    ...
    def run_complete_ml_pipeline(self):
        ...
        logger.info("🚀 DÉMARRAGE DU PIPELINE ML COMPLET - CHAPITRE 85")
        logger.info("=" * 60)
